...
<body>
			...
			<div class="section-top-border">
				...
				<div class="progress-table-wrap">
					<div class="progress-table">
						...
						<div class="table-row">
							<div class="serial"><a href="A_mview?MId=${list.MId}">${list.MId}</a></div>
							<div class="country">${list.MName}</div>
	...
	<c:if test="${paging.page > 1}"><a href="A_mlist?page=${paging.page-1}">[이전]</a></c:if>
	
	<!-- [숫자] -->
	<c:forEach begin="${paging.startPage}" end="${paging.endPage}" step="1" var="i">
		<c:choose>
			<c:when test="${paging.page eq i}">${i}</c:when>
			<c:otherwise><a href="A_mlist?page=${i}">${i}</a></c:otherwise>
		</c:choose>
	</c:forEach>
	
	<!-- [다음] -->
	<c:if test="${paging.page >= paging.maxPage}">[다음]</c:if>
	<c:if test="${paging.page < paging.maxPage}"><a href="A_mlist?page=${paging.page+1}">[다음]</a></c:if>
