...
<body>
	...
	<section class="banner-area relative">
		...
		<div class="container">
			<div class="row justify-content-between align-items-center text-center banner-content">
				<div class="col-lg-12">
					<h1 class="text-white">후기</h1>
					<p>This is the place to write a review</p>
				</div>
			</div>
		</div>
	</section>
			...
			<div class="section-top-border">
				...
				<div class="progress-table-wrap">
					<div class="progress-table">
						...
						<div class="table-row">
							<div class="serial">${reviewList.RENo}</div>
							<div class="country">${reviewList.REId}</div>
							<div class="visit"><td><a href="RV_view?RENo=${reviewList.RENo}&page=${paging.page}">${reviewList.RETitle}</a></td></div>
	...
	<c:if test="${paging.page > 1}"><a href="RV_list?page=${paging.page-1}">[이전]</a></c:if>
	
	<!-- [숫자] -->
	<c:forEach begin="${paging.startPage}" end="${paging.endPage}" step="1" var="i">
		<c:choose>
			<c:when test="${paging.page eq i}">${i}</c:when>
			<c:otherwise><a href="RV_list?page=${i}">${i}</a></c:otherwise>
		</c:choose>
	</c:forEach>
	
	<!-- [다음] -->
	<c:if test="${paging.page >= paging.maxPage}">[다음]</c:if>
	<c:if test="${paging.page < paging.maxPage}"><a href="RV_list?page=${paging.page+1}">[다음]</a></c:if>
						...
						<a href="RV_writeForm" class="genric-btn info-border">후기작성</a>
					</div>
