...
<body>
			...
			<div class="section-top-border">
				...
				<div class="progress-table-wrap">
					<div class="progress-table">
						...
						<div class="table-row">
							<div class="serial">${noticelist.NNo}</div>
							<div class="country"><a href="R_nview?NNo=${noticelist.NNo}&page=${paging.page}">${noticelist.NTitle}</a></div>
							<div class="visit">${noticelist.NDate}</div>
	...
	<c:if test="${paging.page > 1}">
		<a href="R_notice?page=${paging.page-1}"> [이전]</a>
	</c:if>

	<!-- [숫자] -->
	<c:forEach begin="${paging.startPage}" end="${paging.endPage}" step="1"
		var="i">
		<c:choose>
			<c:when test="${paging.page eq i}">${i}</c:when>
			<c:otherwise>
				<a href="R_notice?page=${i}">${i}</a>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<c:if test="${paging.page < paging.maxPage}">
		<a href="R_notice?page=${paging.page+1}"> [다음]</a>
	</c:if>
					</div>
				</div>
			</div>
